...
- **`ClassGroupUtils::getClassGroupId()` rebuilds the class map on every call.** Keep validators cheap — put prefix or character checks before regexes, because they run on the merge hot path.
